...
public class searchsubject extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (role.equals("1")) {
            link = "views/admin/searchsubject.jsp";
        } else {
            link = "views/seesubject.jsp";
        }
        ...
        for (objclass object : list) {
            if (dao.checksubject(object.getSubjectid()) == null) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        accountDBcontext dao = new accountDBcontext();
        ArrayList<objclass> list = dao.loadSubject();
        if (subjectid.length() != 0) {
            list.removeIf(a -> !a.getSubjectid().trim().contains(subjectid.trim()));
        }
        if (subjectname.length() != 0) {
            list.removeIf(a -> !a.getSubjectname().trim().contains(subjectname.trim()));
        }
        if (totalslot.length() != 0) {
            list.removeIf(a -> a.getTotalslot() != Integer.parseInt(totalslot));
        }
        if (semester.length() != 0) {
            list.removeIf(a -> a.getKihoc() != Integer.parseInt(semester));
        }
        if (list.isEmpty()) {
